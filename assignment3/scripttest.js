...
// Pairing icons to their respective animate class
const allIconStates = [
  {
    animCSS: i1x1List,
    icon: icon1x1,
...
// Actual nested hell DO NOT DO THIS
for (let i = 0; i < allIconStates.length; i++) {
  for (let ii = 0; ii < allIconStates[i].icon; ii++) {
    allIconStates[i].icon[ii].addEventListener("click", function anim() {
      animateClass(allIconStates[i].icon[ii], allIconStates[i].animCSS, ii);
    });
  }
}
...
// Every interval, play the idle animation of a random icon

// New list containing all idle animation at index 0 of anim list
// Coupled with the respective icon set

// At this point I realized I could've optimized the previous set CSS animation
// code, using a nested list in this fashion.
// But that would require a complete rehaul so for now I'd keep the code
